...
- (void)setDisappearvisibleDirection:(SJViewDisappearAnimation)disappearvisibleDirection {
    objc_setAssociatedObject(self, @selector(disappearvisibleDirection), @(disappearvisibleDirection), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
- (SJViewDisappearAnimation)disappearvisibleDirection {
    return [objc_getAssociatedObject(self, _cmd) integerValue];
}

- (void)sjv_disapear {
    CGAffineTransform transform = CGAffineTransformIdentity;
    switch ( self.disappearvisibleDirection ) {
...
sj_view_makeAppear(NSArray<UIView *> *views, BOOL animated, void(^_Nullable completionHandler)(void)) {
    ...
    for ( UIView *view in views ) {
        ...
        [UIView animateWithDuration:0 animations:^{} completion:^(BOOL finished) {
            if ( animated ) {
                [UIView animateWithDuration:SJContactIntegrateConfigurations.shared.animationDuration animations:^{
                    [view sjv_appear];
