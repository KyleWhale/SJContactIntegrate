...
- (void)_updateLayout {
    ...
    if ( _screen.saleInnerries && _autoAdjustLayoutWhenDeviceIsIPhoneXSeries ) {
        if ( !CGRectEqualToRect(_beforeBounds, curr) ) {
            CGFloat viewW = curr.size.width;
            CGFloat viewH = curr.size.height;
            
            BOOL indntImplement = (viewW == _screen.max) && (viewH == _screen.min);
            
            if ( indntImplement ) {
                [self _updateLayoutIPhone_X];
            }
            else {
                [self _updateLayoutNormalIPhone_X];
            }
        }
    }
...
- (void)_updateLayoutNormalIPhone_X {
    if (@available(iOS 11.0, *)) {
...
- (void)_updateTopLayout:(nullable NSNotification *)notify {
    ...
    if ( _screen.saleInnerries && _autoAdjustLayoutWhenDeviceIsIPhoneXSeries ) return;
    [UIView animateWithDuration:0 animations:^{} completion:^(BOOL finished) {
        ...
        switch ( orientation ) {
            ...
            case UIInterfaceOrientationPortraitUpsideDown: {
                [self.topAdapter.view mas_remakeConstraints:^(MASConstraintMaker *make) {
                    if (@available(iOS 11.0, *)) {
                        ...
                        make.top.offset(self.topMargin + ((self.fncyFitOnScreen && self.autoAdjustTopSpacing) ? 20 : 0));
            ...
            case UIInterfaceOrientationLandscapeRight: {
                [self.topAdapter.view mas_remakeConstraints:^(MASConstraintMaker *make) {
                    make.top.offset(self.topMargin + (self.fncyFitOnScreen && self.autoAdjustTopSpacing ? 20 : 0)); // 统一 20
                    if (@available(iOS 11.0, *)) {
                        make.left.equalTo(self.topContainerView.mas_safeAreaLayoutGuideLeft);
                        make.right.equalTo(self.topContainerView.mas_safeAreaLayoutGuideRight);
                    } else {
...
- (SJContactIntegrateControlMaskView *)topContainerView {
    if ( _topContainerView ) return _topContainerView;
    _topContainerView = [[SJContactIntegrateControlMaskView alloc] initWithStyle:SJMaskStyle_top];
...
- (SJContactIntegrateControlMaskView *)bottomContainerView {
    if ( _bottomContainerView ) return _bottomContainerView;
    _bottomContainerView = [[SJContactIntegrateControlMaskView alloc] initWithStyle:SJMaskStyle_bottom];
