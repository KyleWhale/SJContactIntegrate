//
//  SJCompressSemicolonRestrictView.m
//  Pods
//
...
#import <SJBaseSequenceInvolve/CALayer+SJBaseSequenceInvolveExtended.h>
#import <SJUIKit/NSAttributedString+SJMake.h>

NS_ASSUME_NONNULL_BEGIN
@interface SJCompressSemicolonRestrictView ()
...
@implementation SJCompressSemicolonRestrictView
@synthesize animating = _animating;
- (instancetype)initWithFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    if ( self ) {
        [self _setupViews];
        [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(_updateSettings) name:SJContactIntegrateConfigurationsDidUpdateNotification object:nil];
...
- (void)setRate:(CGFloat)rate {
    if ( rate != _rate ) {
        ...
        NSAttributedString *text = [NSAttributedString sj_UIKitText:^(id<SJUIKitTextMakerProtocol>  _Nonnull make) {
            make.append([NSString stringWithFormat:@"%.01fx", rate]).textColor(sources.speedupContentDataRateTextColor).font(sources.speedupContentDataRateTextFont);
            if ( strings.longPressSpeedupContentData.length != 0 ) {
                make.append(@" ");
                make.append(strings.longPressSpeedupContentData).font(sources.speedupContentDataTextFont).textColor(sources.speedupContentDataTextColor);
            }
        }];
...
- (void)_recursiveAnimationWithBeginIndex:(NSInteger)idx {
    ...
    [layer addAnimation:[self _opacityAnimationWithDuration:0.3] stopHandler:^(CAAnimation * _Nonnull anim, BOOL finished) {
...
- (void)_updateSettings {
    ...
    for ( CAShapeLayer *layer in _triangles ) {
        layer.strokeColor = layer.fillColor = sources.speedupContentDataTriangleColor.CGColor;
    }
}
@end
