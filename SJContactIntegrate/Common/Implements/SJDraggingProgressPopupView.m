...
#if __has_include(<SJBaseSequenceInvolve/NSString+SJBaseSequenceInvolveExtended.h>)
#import <SJBaseSequenceInvolve/NSString+SJBaseSequenceInvolveExtended.h>
#else
...
- (void)setDragTime:(NSTimeInterval)dragTime {
    ...
    _dragTimeLabel.text = [NSString stringWithContentTime:dragTime duration:_duration];
    _dragTime = dragTime;
}

- (void)setContentTime:(NSTimeInterval)time {
    if ( _contentTime != time ) {
        _contentTime = time;
        _progressSlider.bufferProgress = time / _progressSlider.maxValue;
    }
}
...
- (void)setDuration:(NSTimeInterval)duration {
    if ( _duration != duration ) {
        ...
        _durationLabel.text = [NSString stringWithContentTime:duration duration:duration];
    }
}
